...
-- TRIGGER ARE EXECUTED AUTOMATICALLY WHEN A DML OPERATION
-- LIKE INSERT,UPDATE AND DELETE ARE EXECUTED
-- TWO TYPES OF TRIGGER 
-- 1) BEFORE TRIGGER -- EXECUTE BEFORE DML 
-- 2) AFTER TRIGGER -- EXECUTE AFTER DML


-- BEFORE TRAIGGER EXAMPLE
-- SUPPOSE YOU ENTER FIRSTNAME AND LASTNAME IN A TABLE
-- WHEN YOU DO THE NAME SOULD BE IN CORRECT FORMAT LIKE 
-- SUPPOSE YOU ENTER 'TAnVIR' AND IT SHOULD BE LIKE 'Tanvir'
-- SO THIS COULD BE DONE WITH A TRIGGER BEFORE INSERTING IN A 
-- TABLE . WHEN YOU ARE GOING TO INSERT FIRST THE BEFORE TRIGGER FORMAT 
-- YOUR NAME THEN IT INSERT IT


-- AFTER TRIGGER 
-- SUPPOSE YOU WANT TO SAVE THE TIME OF INSERT IN ANOTHER TABLE 
-- YOU CAN USE AFTER TRIGGER.
-- WHEN YOU INSERT A DATA THE AFTER TRIGGER WILL 
-- WILL EXECUTE ANOTHER COMMAND THAT YOU WROTE 
-- THAT WILL SAVE THE TIME OF THE INSERT


-- TRIGGER IS A ROW LEVEL OPERATION
-- YOU HIT A SPECIFIC ROW INSTEAD OF AN ENTIRE COLUMN
-- SO TRIGGER MUST BE CREATED NWITH 'FOR EACH ROW' CLAUSE
-- BECAUSE TRIGGER A RE ROW LEVEL

-- OLD KEYWORD
-- OLD KEYWORD GETS A VALUE FROM THE ROWS THAT IS BEING UPDATED OR DELETED
-- NEW KEYWORD GETS A VALUE FROM ROW THAT IS INSERTER OR UPDATED
-- WE WILL GET TO THAT LATER IN CODE
...
-- SUPPOSE YOU FORMAT BEFORE INSERT 
-- 1) SO IT IS A BEFORE TRIGGER
-- 2) YOU AR GOING TO INSERT NEW VALUE
-- 3) SO THE KEYWORD WILL BE NEW
-- 4) YOU WANT TO INSER IN THE 'NAME' COLUMN
-- 5) YOU WILL WRITE 'NEW.NAME' IN ALL CODE

USE SAKILA;
SELECT * FROM LANGUAGE;
-- SEPARATE TEST BEFORE CREATING TRIGGER
-- SUBSTRING(string, start, length)
SELECT SUBSTRING("tanVIR",1,1);
...
-- NOW INSERT SOME LANGUAGE NAME WITHOUT PROPER FORMAT
-- SEE WHAT FORMAT IS SAVING

INSERT INTO language (name) VALUES ('aRabiC');
INSERT INTO language (name) VALUES ('gUZRATI');
INSERT INTO language (name) VALUES ('tamil');
INSERT INTO language (name) VALUES ('gerMan');
SELECT * FROM language; -- all are the correct format
SHOW Triggers;
DROP TRIGGER FORMATBEFOREINSERT; 




-- CREATE A NEW TABLE 
CREATE TABLE UPDATE_INFO(old_name VARCHAR(100),updated_name VARCHAR(100),update_time DATETIME);

-- new every update in language table 
-- this UPDATE_TABLE will be filled with 
-- the old name new name and the time of that update
-- you can create trigger  in after_insert too of you want
DELIMITER //
...
UPDATE language
SET name = "British English"
WHERE language_id = 1;
-- now update one value of the language
-- working perfectly
SELECT * FROM UPDATE_INFO;